...
class ClusterGCNScheduler(IScheduler):
    ...
    def __init__(self, cloud=None, driver=None, environment=None):
        ...
        model_path = 'gnn_training_data/cluster_gcn_model.pth'
        scorer_path = 'gnn_training_data/cluster_gcn_scorer.pth'
        vm_scaler_path = 'gnn_training_data/vm_scaler.pkl'
        pm_scaler_path = 'gnn_training_data/pm_scaler.pkl'

        if not all(os.path.exists(p) for p in [model_path, scorer_path, vm_scaler_path, pm_scaler_path]):
        ...
        info(f"Cluster-GCN Scheduler initialized correctly on device: {self.device}.")
    ...
    def reevaluate(self):
        ...
        for vm_idx, vm in enumerate(vms_to_schedule):
            for pm_idx in sorted_pm_indices[vm_idx]:
                ...
                info(f"ClusterGCN-Scheduler: No suitable PM found for VM {vm.id}")
