...
USAGE_LOC = os.path.realpath(os.path.join(os.path.dirname(__file__), '../..', USAGE_LOC))
DATA_LOC = "io/geotemp/" # path from the philharmonic root
# from this module to philharmonic root and then to DATA_LOC
DATA_LOC = os.path.realpath(os.path.join(os.path.dirname(__file__), '../..', DATA_LOC))
...
start = pd.Timestamp('2010-06-03 00:00:00')
...
# - five minutes
# times = pd.date_range(start, periods=60, freq='min')

end = times[-1]

# plotting results
# plot_on_serve = True
plot_on_server = False
if plot_on_server:  # plotting in Server GUI-less environment
    liveplot = False
    fileplot = True
else:               # plotting in GUI environment(Desktop OS)
    liveplot = False
    fileplot = False
...
factory = {
    ...
    #  FBFScheduler, BFDScheduler, GAScheduler, NoScheduler, GGCNNBasedScheduler
...
inputgen_settings = {
    ...
    'VM_request_generation_method': 'normal_vmreqs',
    # 'VM_num': 20,
    'VM_num': 40,
